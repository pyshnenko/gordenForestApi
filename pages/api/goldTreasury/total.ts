                    ...
                    if ((dat[0].role==='Secretary')||(dat[0].role==='Lord')||(dat[0].role==='Treasurer')) {
                        if (req.hasOwnProperty('body')&&(typeof(req.body)==='object')&&(Object.keys(req.body).length>=1)){
                            ...
                            if ((buf.hasOwnProperty('login'))&&(typeof(buf.login)==='string')&&(buf.login!=='')){
                                let result = await mongoS.goldTotal(dat[0].login, buf.login, true, buf.addr || 0);
                                res.status(200).json({res: result});
                            }
                            else {
                                let result = await mongoS.goldTotal(dat[0].login, undefined , true, buf.addr || 0);
                                res.status(200).json({res: result});
                            }
                        }
                        else {
                            let result = await mongoS.goldTotal(dat[0].login, undefined , true, 0);
                            res.status(200).json({res: result});
                        }
                    }
                    else {
