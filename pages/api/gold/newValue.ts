...
log4js.configure({
    appenders: { 
        cApi: { type: "file", filename: "log/newGoldValue.log" }, 
...
export default async function handler(req: any, res: any) {
    logger.info(req.method + ' newGoldValue');
        ...
        if ((req.headers?.authorization!==undefined)&&(req.hasOwnProperty('body')))
            {
                ...
                if (dat.length) {
                    if ((dat[0].role==='Secretary')||(dat[0].role==='Lord')||(dat[0].role==='Treasurer')) {
                        ...
                        let result = await mongoS.newGoldValue({login: String(buf.login), value: Number(buf.value), date: Number(buf.date), veryfi: dat[0].login, way: buf.way || ''}, dat[0].login);
