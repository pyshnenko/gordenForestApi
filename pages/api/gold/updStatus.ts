...
log4js.configure({
    appenders: { 
        cApi: { type: "file", filename: "log/goldUPDSTATUS.log" }, 
...
export default async function handler(req: any, res: any) {
    logger.info(req.method + ' goldUPDSTATUS');
        ...
        if ((req.headers?.authorization!==undefined)&&(req.hasOwnProperty('body')))
            {
                ...
                if (dat.length) {
                    if ((dat[0].role==='Secretary')||(dat[0].role==='Lord')) {
                        ...
                        let result = await mongoS.updSaleStatus({login: String(buf.login), sale: Number(buf.sale), status: Number(buf.status)}, dat[0].login);
